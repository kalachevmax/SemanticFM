 ...
 * @return {fm.Action}
 */
fm.STRING = function(value) {
  function newString(complete, cancel, input) {
    if (typeof value === 'string') {
      complete(value);
    } else {
      cancel('[fm.STRING] value type is incorrect: ', value);
    }
  }
 ...
 * @return {fm.Action}
 */
fm.NUMBER = function(value) {
  function newNumber(complete, cancel, input) {
    if (typeof value === 'number') {
      complete(value);
    } else {
      cancel('[fm.NUMBER] value type is incorrect: ', value);
    }
  }
...
/**
 * @param {fm.Type} type
 * @param {!Array.<fm.Atom>} value
 * @return {fm.Action}
 */
fm.LIST = function(type, value) {
  function newList(complete, cancel, input) {
    if (value instanceof Array && fm.arrayOf(type, value)) {
      complete(value);
    } else {
      cancel('[fm.LIST] value type is incorrect: ', value);
    }
  }
 ...
 * @return {fm.Action}
 */
fm.LIST_STRING = function(value) {
  return fm.LIST(fm.Type.STRING, value);
};
...
fm.registerTypeProvider(new fm.ListProvider());
