...
fm.calc = function(symbol) {
  ...
  function calc(args) {
     ...
     * @param {fm.Input=} opt_input
     */
    function action(complete, cancel, opt_input) {
      if (typeof fm.__symbols[symbol] === 'function') {
        fm.__symbols[symbol](args, function() {
          complete(opt_input);
        }, cancel);
      } else {
        cancel('[fm.calc] symbol ' + symbol + 'is not defined');
      }
    }
