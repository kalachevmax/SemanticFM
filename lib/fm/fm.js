 ...
 * @return {fm.SymbolValue}
 */
fm.getSymbol = function(name) {
  return fm.__symbols[name] || null;
};
 ...
 * @return {boolean}
 */
fm.removeSymbol = function(name) {
  if (typeof fm.__symbols[name] !== 'undefined') {
    fm.__symbols[name] = null;
    return true;
  }
...
/**
 * @param {!Array.<!fm.Box|fm.SymbolValue>} symbols
 * @return {!Array.<*>}
 */
fm.unboxArray = function(symbols) {
  var result = [];
  ...
  while (i < l) {
    result.push(symbols[i] instanceof fm.Box ? symbols[i].get() : symbols[i]);
    i += 1;
  }
 ...
 */
fm.addKey = function(hashMapName, keyAction) {
  /**
   * @param {function(string)} complete
   * @param {fm.Cancel} cancel
   * @param {fm.SymbolValue} value
   */
  function addKey(complete, cancel, value) {
    keyAction(function(key) {
      if (typeof fm.__symbols[hashMapName] !== 'undefined') {
        fm.__symbols[hashMapName][key] = value;
        complete(key);
      } else {
...
fm.getValue = function(hashMapName) {
   ...
   * @param {string} key
   */
  function action(complete, cancel, key) {
    if (typeof fm.__symbols[hashMapName] !== 'undefined') {
      complete(fm.__symbols[hashMapName][key]);
    } else {
 ...
 * @return {fm.Action}
 */
fm.action = function(syncFn) {
  return function(complete, cancel, input) {
    complete(syncFn(input) || null);
  }
};
...
fm.pass = function(action, opt_actionInput) {
   ...
   * @param {fm.Input} input
   */
  function pass(complete, cancel, input) {
    action(function() {
      complete(input);
    }, cancel, opt_actionInput || null);
  }
