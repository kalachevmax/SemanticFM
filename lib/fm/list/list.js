...
/**
 * @param {fm.RegularAction} action
 * @param {fm.Input} base
 * @return {fm.Action|function(function(fm.Complete, fm.Cancel, fm.List))}
 */
fm.list.fold = function(action, base) {
  /**
  ...
  function fold(complete, cancel, list) {
    ...
    function process(item) {
      action(handleProcessed, cancel, item);
    }
